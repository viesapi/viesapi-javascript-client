 ...
 * @param {import("@xmldom/xmldom").Document} doc XML document
 * @param {string} path xpath string
 * @return {string} element value
 */
function xpathString(doc, path)
{
    // @ts-expect-error TS2345: Document extends Node
    ...
    // @ts-expect-error TS2339: ignore this
	if (nodes.length !== 1) {
		return '';
	}
...
function toString(date)
{
	if (typeof date === 'string') {
        ...
        // @ts-expect-error TS2345: isNaN(date)
 ...
 * @returns {import("@xmldom/xmldom").Document|null} XML document or null
 */
 ...
 * @param {VIESAPIClient} viesapi client object
 * @param {string} url target URL
 * @return {Promise<import("@xmldom/xmldom").Document>} promise returning XML document on success
 */
 ...
 * @param {import("@xmldom/xmldom").Document} doc document object
 * @param {string} prefix XPath prefix
 * @returns {VIESData} data object
 */
function parse(doc, prefix)
{
    const vd = new VIESData();

    vd.uid = xpathString(doc, prefix + '/uid/text()');
    vd.countryCode = xpathString(doc, prefix + '/countryCode/text()');
    vd.vatNumber = xpathString(doc, prefix + '/vatNumber/text()');
    vd.valid = (xpathString(doc, prefix + '/valid/text()') === 'true');
    vd.traderName = xpathString(doc, prefix + '/traderName/text()');

    const name = xpathString(doc, prefix + '/traderNameComponents/name/text()');

    if (name) {
        const nc = new NameComponents();
        nc.name = name;
        nc.legalForm = xpathString(doc, prefix + '/traderNameComponents/legalForm/text()');
        nc.legalFormCanonicalId = xpathInt(doc, prefix + '/traderNameComponents/legalFormCanonicalId/text()');
        nc.legalFormCanonicalName = xpathString(doc, prefix + '/traderNameComponents/legalFormCanonicalName/text()');

        vd.traderNameComponents = nc;
    }

    vd.traderCompanyType = xpathString(doc, prefix + '/traderCompanyType/text()');
    vd.traderAddress = xpathString(doc, prefix + '/traderAddress/text()');

    const country = xpathString(doc, prefix + '/traderAddressComponents/country/text()');

    if (country) {
        const ac = new AddressComponents();
        ac.country = country;
        ac.postalCode = xpathString(doc, prefix + '/traderAddressComponents/postalCode/text()');
        ac.city = xpathString(doc, prefix + '/traderAddressComponents/city/text()');
        ac.street = xpathString(doc, prefix + '/traderAddressComponents/street/text()');
        ac.streetNumber = xpathString(doc, prefix + '/traderAddressComponents/streetNumber/text()');
        ac.houseNumber = xpathString(doc, prefix + '/traderAddressComponents/houseNumber/text()');
        ac.other = xpathString(doc, prefix + '/traderAddressComponents/other/text()');

        vd.traderAddressComponents = ac;
    }

    vd.id = xpathString(doc, prefix + '/id/text()');
    vd.date = xpathDate(doc, prefix + '/date/text()');
    vd.source = xpathString(doc, prefix + '/source/text()');
...
VIESAPIClient.prototype.getVIESDataAsync = function(numbers) {
	return new Promise((resolve, reject) => {
		...
		// validate input
		if (numbers.length < 3 || numbers.length > 99) {
...
VIESAPIClient.prototype.getVIESDataAsyncResult = function(token) {
	return new Promise((resolve, reject) => {
		...
		get(this, this.url + '/batch/vies/' + token).then((doc) => {
            ...
            br.uid = xpathString(doc, '/result/batch/uid/text()');
			...
			for (let i = 1; ; i++) {
				...
				br.numbers.push(parse(doc, '/result/batch/numbers/vies[' + i + ']'));
			}
 ...
 */
VIESAPIClient.prototype.getVIESStatus = function() {
    return new Promise((resolve, reject) => {
        // clear error
        clear(this);

        // send request
        get(this, this.url + '/check/vies/status').then((doc) => {
            const vs = new VIESStatus();

            vs.uid = xpathString(doc, '/result/vies/uid/text()');
            vs.available = (xpathString(doc, '/result/vies/available/text()') === 'true');

            for (let i = 1; ; i++) {
                const code = xpathString(doc, '/result/vies/countries/country[' + i + ']/countryCode/text()');

                if (!code) {
                    break;
                }

                const cs = new CountryStatus();

                cs.countryCode = code;
                cs.status = xpathString(doc, '/result/vies/countries/country[' + i + ']/status/text()');

                vs.countries.push(cs);
            }
